...
Fixpoint subst (x:string) (s:tm) (t:tm) : tm :=
  ...
  | tsnd t2 => tsnd (subst x s t2)
  | tunit => tunit
  | tlet y t1 t2=> tlet y (subst x s t1) (if beq_string x y then t2 else (subst x s t2))
  | tinl T t1 =>
      tinl T (subst x s t1)
  | tinr T t1 =>
      tinr T (subst x s t1)
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_Fix1 : forall t1 t1',
       t1 ==> t1' ->
       tfix t1 ==> tfix t1'
  | ST_Fix2 : forall x T t1 t2,
       t2 = tabs x T t1 ->
       tfix t2 ==> [x := tfix t2] t1
...
Proof with eauto.
  ...
  - (* T_Pair *)
    ...
    + right. inversion H as [t1' Hstp]. exists (tpair t1' t2)...
  - (* T_Fst *)
    right. destruct IHHt...
    + inversion H; subst; try solve_by_invert. exists v1...
    + inversion H as [t1 Hstp]. exists (tfst t1)...
  - (* T_Snt *)
    right. destruct IHHt...
    + inversion H; subst; try solve_by_invert. exists v2...
    + inversion H as [t1 Hstp]. exists (tsnd t1)...
  - (* T_Unit *)
    left...
    (* let *)
    (* FILL IN HERE *)
  ...
  - (* fix *)
    right. destruct IHHt...
    + inversion H; subst; try solve_by_invert.  exists ([x:=tfix (tabs x T11 t12)]t12)...
    + destruct H...
...
(** $Date: 2017-11-21 13:51:42 -0500 (Tue, 21 Nov 2017) $ *)
