...
*)
(** f: For a student get the total number of days attended(nat)
    g: Applying f but without the number of days which is needed as Student is a subtype
        of Person. *)
...
Between [Top->Student] and [Student->Top]
*)
(**  *)
...
(** Is the following statement true or false?  Briefly explain your
    ...
    forall T,
         ...
         exists S,
            S <: T  /\  S <> T
*)
(** The given statement is false because it fails for TUnit *)
...
*)
(** No this is not a good idea because it can get stuck if the fst and snd are not passed
    correctly like in the case of arrow_sub_wrong. *)
  ...
  - (* T_App *)
    ...
    + (* t1 is a value *)
      ...
      * (* t2 is a value *)
        ...
        subst. exists ([x:=t2]t12)...
      * (* t2 steps *)
        inversion H0 as [t2' Hstp]. exists (Top.tapp t1 t2')...
    + (* t1 steps *)
      inversion H as [t1' Hstp]. exists (Top.tapp t1' t2)...
  - (* T_If *)
    right.
    destruct IHHt1.
    + (* t1 is a value *) eauto.
    + assert (t1 = Top.ttrue \/ t1 = Top.tfalse)
        by (eapply canonical_forms_of_Bool; eauto).
      inversion H0; subst...
    + inversion H. rename x into t1'. eauto. 
Qed.
...
(** $Date: 2017-12-05 13:33:15 -0500 (Tue, 05 Dec 2017) $ *)
