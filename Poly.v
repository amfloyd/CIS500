...
Definition filter_even_gt7 (l : list nat) : list nat:=
  filter (fun e=> andb (evenb e) (leb 7 e)) l.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X:=
  (filter test l, filter ( fun f => negb(test f) )l).
...
Lemma aux: forall (X Y : Type) (f: X-> Y) (l1 l2 : list X),
map f (l1 ++ l2) = map f (l1) ++ map f (l2).

Proof. intros X Y f l1 l2. induction l1 as [| x l' IHl'].
- simpl. reflexivity.
- simpl. rewrite -> IHl'. reflexivity. Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. rewrite -> aux. reflexivity. Qed. 
(** [] *)
...
Proof. intros X Y f l. induction l as [| h l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. reflexivity. Qed.
