...
Theorem even5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros. inversion H. inversion H1 as [| n' H2]. inversion H2.
Qed. 
(** [] *)
...
If we drop c5, the set will not change because [R 1 1 2] is proved by c1, c2, c3.

If we drop c4, the set will not change because [R 1 1 2] is proved by c1, c2, c3.
...
Lemma MUnion' : forall T (s : list T) (re1 re2 : reg_exp T),
  ...
  s =~ Union re1 re2.
Proof.
  intros. inversion H. apply MUnionL. apply H0. apply MUnionR. apply H0.
Qed.
...
intros. induction s2.
  - simpl. split. intros. inversion H. 
    reflexivity. intros. rewrite H. apply MEmpty.
  - split. intros. simpl in H. inversion H. inversion H3. rewrite <- H2 in IHs2. rewrite H7 in H5. rewrite <- H5 in IHs2.
    simpl. apply f_equal. inversion IHs2.
Admitted.
...
Proof.
  ...
  - intros. induction re.
    ...
    + simpl. reflexivity.
    + simpl. reflexivity.
    + simpl. apply andb_true_iff. split. apply IHre1. destruct H.
      inversion H. exists s1. apply H3. apply IHre2. destruct H.
      inversion H. exists s2. apply H4.
    + simpl. apply orb_true_iff. destruct H. inversion H. left.
      apply IHre1. exists x. apply H2. right. apply IHre2. exists x.
      apply H2.
    + simpl. reflexivity.
  - intros. induction re.
    ...
    + simpl in H. apply andb_true_iff in H. inversion H.
      apply IHre1 in H0. apply IHre2 in H1. inversion H0. inversion H1.
...
intros. remember (Star re) as re'. induction H.
  ...
  - inversion Heqre'.
  - inversion Heqre'.
  - inversion Heqre'. exists []. simpl. split. reflexivity.
    intros. inversion H.
  - exists ([s1] ++ [s2]). simpl. rewrite app_nil_r. split. reflexivity.
...
Proof.
  ...
  - inversion H. split. intros. unfold not in H0. apply H0 in H1. inversion H1.
    intros. inversion H1.
Qed.
...
Proof.
  intros x y.
 unfold not. intros H. destruct (beq_nat x y) eqn: G. apply beq_nat_true in G. apply H in G. inversion G.
  reflexivity.
Qed.
...
Proof.
  ...
  - simpl. simpl in H. destruct (beq_natP x n).
    + rewrite H0. intros. rewrite H0 in H. 
      assert (G: forall n' : nat, beq_nat n' n' = true). { induction n'. simpl. reflexivity. simpl. apply IHn'. }
      rewrite G in H. simpl in H. inversion H.
    + unfold not. intros. inversion H1. apply H0 in H2. apply H2. apply IHl. apply (beq_nat_false) in H0. assert (G: beq_nat x n = beq_nat n x).
      { destruct beq_nat. rewrite H0. inversion H. rewrite H0. reflexivity. } rewrite <- G in H. rewrite H0 in H. simpl in H. apply H. apply H2.
Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | no : nostutter []
  | one : forall x: X, nostutter [x]
  | two : forall (h1 h2 : X) (l:list X) (P: h1 <> h2), nostutter (h2:: l) -> nostutter (h1 :: h2 :: l).
...
Example test_nostutter_4: not (nostutter [3;1;1;4]).
  ...
  Proof. intro. inversion H. inversion H1. inversion H5. inversion H9. destruct P0. reflexivity.
  Qed.
