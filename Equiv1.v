...
Proof.
  ...
  - inversion E.
    + subst. clear E. unfold bequiv in H.
      rewrite H in H5. simpl in H5. inversion H5.
    + subst. clear E. auto.
  - apply E_IfFalse.
    + unfold bequiv in H. simpl in H. auto.
    + auto.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion H; subst.
    + apply E_IfFalse. simpl. rewrite H5.
      simpl. reflexivity. apply H6.
    + apply E_IfTrue. simpl. rewrite H5.
      simpl. reflexivity. apply H6.
  - inversion H; subst.
...
Proof.
  (*  intros. unfold not. intros C.
      inversion C. -> will get you stuck *)
  (* WORKED IN CLASS *)
  intros b c st st' Hb.
  intros H.
...
Proof.
  ...
  - apply WHILE_true_nonterm in H.
    exfalso. apply H. apply Hb.
  - apply WHILE_true_nonterm in H.
    exfalso. apply H. simpl. unfold bequiv; intros. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - (* -> *)
    ...
    + (* loop runs *)
      ...
      apply E_Seq with (st' := st'0). (* or use eapply E_Seq. *)
...
Proof.
  ...
  - replace st' with (t_update st' X (aeval st' X)).
    + inversion H; subst. apply E_Ass. unfold aequiv in He. auto.
    + apply functional_extensionality. intros.
      rewrite t_update_same. reflexivity.
  - inversion H; subst. rewrite <- He. simpl. replace (t_update st X (st X)) with st.
    + constructor.
    + apply functional_extensionality. intros.
      rewrite t_update_same. reflexivity.
Qed.
...
Definition equiv_classes : list (list com) :=
  [[prog_a;prog_d];[prog_b;prog_e;prog_f];[prog_c;prog_h;prog_g];[prog_i]].
(** [] *)
...
Proof.
  ...
  rename H0 into Hc1. rename H1 into Hc2.
  split; intros Hif.
  - remember (IFB b THEN c1 ELSE c2 FI) as cif eqn:Hcif.
    induction Hif; inversion Hcif; subst.
    + apply E_IfTrue.
  ...
  - remember (IFB b' THEN c1' ELSE c2' FI) as c'If eqn:Hc'If.
    induction Hif; inversion Hc'If; subst.
    + apply E_IfTrue.
      * rewrite H. assumption.
      * apply Hc1. assumption.
    + apply E_IfFalse.
...
Proof.
  ...
  - (* WHILE *)
    ...
    apply WHILE_true. apply H. apply WHILE_false. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - remember (WHILE BTrue DO SKIP END) as cwhile.
  induction Hs; inversion Heqcwhile; subst.
  inversion H0.
