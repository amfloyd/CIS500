...
Theorem t_update_same : forall X x (m : total_map X),
    m & { x --> m x } = m.
  Proof.
  intros X x m. simpl. unfold t_update. apply functional_extensionality. intros x0.
  destruct (beq_string x x0) eqn:H.
  - Search f_equal. apply f_equal. Search beq_string. apply beq_string_true_iff in H. apply H.
  - reflexivity.
Qed.
(** [] *)
...
Theorem t_update_permute : forall (X:Type) v1 v2 x1 x2
  ...
  =  m & { x1 --> v1 ; x2 --> v2 }.
Proof.
  intros. unfold t_update. apply functional_extensionality. intros x. destruct (beq_string x1 x) eqn:H1.
  - destruct (beq_string x2 x) eqn:H2.
  + apply beq_string_true_iff in H1. apply beq_string_true_iff in H2. destruct H. rewrite -> H1. rewrite -> H2.
    reflexivity.
  + reflexivity.
  - reflexivity.
Qed.
