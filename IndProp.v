...
Proof.
  intros n m E. intros H. induction E as [| n' E'].
  - simpl. apply H.
...
The set of provable propositions will not change if we drop [c5] because the provable proposition [R 1 1 2], can be 
proved by using c1, c2, c3.

The set of provable propositions will not change if we drop [c4] because the provable proposition [R 1 1 2], can be 
proved by using c1, c2, c3.
...
Lemma empty_is_empty : forall T (s : list T),
  ~ (s =~ EmptySet).
Proof.
  intros T s. unfold not. intros H. inversion H. Qed.

Lemma MUnion' : forall T (s : list T) (re1 re2 : @reg_exp T),
  s =~ re1 \/ s =~ re2 ->
  s =~ Union re1 re2.
Proof.
  intros T s rel1 rel2. intros [H1 | H2].
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  ...
  fold app ss [] =~ Star re.
Proof.
  intros T ss re. intros H. induction ss as [| ss' IHss'].
  - simpl. apply MStar0.
  - simpl. apply MStarApp.
    { apply H. simpl. left. reflexivity.
    }
    { apply IHIHss'. intros T1 H1. apply H. simpl. right. apply H1.
    }
Qed.
(** [] *)
...
Proof.
  ...
  + simpl. reflexivity.
  + simpl. reflexivity.
  + simpl. Search andb. apply andb_true_iff. split.
    { apply IHre1. destruct H. inversion H. exists s1. apply H3.
    }
    { apply IHre2. destruct H. inversion H. exists s2. apply H4.
    }
  + simpl. Search orb. apply orb_true_iff.
  ...
  - intros H. induction re.
    ...
    + simpl in H. simpl. exists []. apply MEmpty.
    + simpl in H. exists [t]. apply MChar.
...
Proof.
  ...
  - inversion H. split.
    ...
    + intros H1. apply H0.
  - inversion H. split.
    + unfold not in H0. intros H1. apply H0 in H1. inversion H1.
    + unfold not in H0. intros H1. inversion H1.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. inversion H. unfold not. intros. apply H0.
  - simpl. unfold not. intros H1. simpl in H. destruct (beq_natP n m) as [H2 | H2].
    + inversion H.
    + simpl in H1. destruct H1.
      { rewrite H0 in H2. apply H2. reflexivity. }
      { apply IHl' in H0.
        { apply H0. }
        {  simpl in H. apply H. }
      }
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | none : nostutter[]
  | once : forall x:X, nostutter[x]
  | twice : forall (x y:X) (l: list X ), x<>y -> nostutter(y :: l) -> nostutter(x :: y :: l).
...
(** $Date: 2017-09-28 17:25:59 -0400 (Thu, 28 Sep 2017) $ *)
