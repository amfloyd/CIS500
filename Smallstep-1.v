  ...
  induction Hy1; intros y2 Hy2.
    ...
    - (* ST_Plus1 *) inversion Hy2.
      ...
      + (* ST_Plus1 *)
        ...
        reflexivity. assumption.
      + (* ST_Plus2 *) 
        rewrite<-H in Hy1. rewrite<-H in H1. try subst. try assumption.
        inversion H1. try subst. try assumption. inversion Hy1.
    - (* ST_Plus2 *) inversion Hy2.
      + (* ST_PlusConstConst *) 
        try subst. inversion Hy1.
      + (* ST_Plus1 *) try subst. inversion H. try subst. inversion H3.
      + (* ST_Plus2 *)
        rewrite <- (IHHy1 t2'0).
        reflexivity. assumption.
Qed.
...
Lemma test_multistep_4:
  ...
  ==>*
      P
        ...
        (C (2 + (0 + 3))).
Proof.
  eapply multi_step with (P(C 0)(P(C 2)(C(0+3)))).
  - constructor.
    + try constructor.
    + try constructor.
      { try constructor. 
...
Proof.
  ...
  - apply multi_refl.
  - eapply multi_step.
    + apply ST_Plus2. try constructor. try assumption. eassumption.
...
Proof.
  intros. induction H.
  - try constructor.
  - apply multi_trans with (P (C n1) t2). apply multistep_congr_1. try assumption.
    + apply multi_trans with (P (C n1) (C n2)).
      * apply multistep_congr_2.
        { try constructor. }
        { try constructor. try assumption. }
      * try constructor. econstructor.
        { try constructor. }
        { try constructor. }
Qed.
(** [] *)
...
intros t t' n Hs. generalize dependent n.
  induction Hs; try subst; intros.
  - inversion H. apply E_Plus; try constructor.
  - inversion H. try constructor.
    + try assumption. try subst. apply IHHs in H2. apply H2.
    + apply H4.
  - inversion H0. try constructor.
    + try assumption.
    + apply IHHs in H5. apply H5.
Qed.
(** [] *)
...
Proof.
  ...
  - induction H.
    + subst. constructor.
    + subst. try constructor. eapply step__eval.
      * apply H.
      * apply IHmulti.
        { apply H0. }
        { reflexivity. }
...
(** $Date: 2017-10-13 17:21:17 -0400 (Fri, 13 Oct 2017) $ *)
