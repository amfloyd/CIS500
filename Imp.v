...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BLe : forall e1 e2 n1 n2,(e1 \\ n1) -> (e2 \\ n2) -> BLe e1 e2 \\\ leb n1 n2
  | E_BNot : forall e n, e\\\n -> BNot e \\\ negb n
  | E_BAnd : forall e1 e2 n1 n2,(e1 \\\ n1) -> (e2 \\\ n2) -> BAnd e1 e2 \\\ andb n1 n2
...
Proof.
  ...
  try (apply aeval_iff_aevalR); try (apply IHb); try (apply IHb1);
...
(** To make Imp programs easier to read and write, we introduce some
    ...
    You do not need to understand these declarations to follow this
    chapter.  Briefly, though, the [Coercion] declaration in Coq
    stipulates that a function (or constructor) can be implicitly used
    by the type system to coerce a value of the input type to a value
    of the output type.  For instance, the coercion declaration for
    [AId] allows us to use plain strings when an [aexp] is expected;
...
Theorem XtimesYinZ_spec : forall st x y st',
  ...
  st' Z = (x * y).
Proof.
  intros st x y st' HX Heval H. inversion H. subst. simpl.
  apply t_update_eq.  Qed.
...
Proof.
  ...
  - apply IHcontra2. simpl. reflexivity.
Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | no_whilesR_add: forall b1 b2, no_whilesR (b1 ::= b2)
  | no_whiles_andb: forall b1 b2, no_whilesR b1 -> no_whilesR b2 -> no_whilesR (b1 ;; b2)
  | no_whiles_ifb: forall b1 ct cf, no_whilesR ct -> no_whilesR cf -> no_whilesR (IFB b1 THEN ct ELSE cf FI).
...
Proof.
  split.
  - intros H. intros. induction c; simpl in H;try(apply andb_true_iff in H; destruct H);
    try(constructor);try (apply IHc1);try (apply IHc2);try(assumption). inversion H.
...
Definition s_eval (st : state) (stack : list nat)
                   (prog : sinstr) :list nat :=
match prog with
| SPush n => n::stack
| SLoad x => st x::stack
...
(** $Date: 2017-10-10 11:42:28 -0400 (Tue, 10 Oct 2017) $ *)
