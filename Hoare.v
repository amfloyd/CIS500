...
eapply hoare_consequence_pre.
  - apply hoare_asgn.
  - unfold assert_implies. unfold assn_sub. intros st H. try simpl.
    rewrite t_update_eq. try assumption.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    + eapply hoare_asgn.
    + intros st H. split.
      { try simpl. unfold t_update. try subst. reflexivity.
      }
      { try simpl. apply t_update_eq.
      }
Qed.
(** [] *)
...
Proof.
  ...
  - eapply hoare_seq.
    + eapply hoare_asgn.
    + eapply hoare_asgn.
  - intros st at' H H'. inversion H. try subst. unfold assn_sub. simpl.
    rewrite t_update_eq. rewrite t_update_neq.
    + rewrite t_update_eq. rewrite t_update_neq. rewrite t_update_eq. rewrite t_update_neq.
      { try assumption.
      }
      { try assumption. unfold not. intros. inversion H0.
      }
      { try assumption. unfold not. intros. inversion H0.
      }
    + try assumption. unfold not. intros. inversion H0.
...
Proof.
  unfold hoare_triple. intros. inversion H; subst. apply H0.
Qed.

End Himp.
(** [] *)

(** $Date: 2017-10-26 16:22:04 -0400 (Thu, 26 Oct 2017) $ *)
