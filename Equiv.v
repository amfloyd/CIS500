...
Proof.
  intros. split; intros.
  - inversion H. subst; try assumption. try subst.
    + apply E_IfFalse; try simpl.
      { apply negb_false_iff. rewrite H5. reflexivity.
      }
      { apply H6.
      }
    + apply E_IfTrue; try simpl.
  ...
  - inversion H. subst; try assumption. try subst.
    + apply E_IfFalse.
      { simpl in H5. subst. try assumption. apply negb_true_iff in H5. apply H5.
      }
      { apply H6.
      }
    + apply E_IfTrue. simpl in H5.
      { apply negb_false_iff in H5. rewrite H5. simpl. reflexivity.
      }
      { apply H6.
      }
Qed.
...
Proof.
  intros b c Hb st st'.
  split;intros.
  - apply WHILE_true_nonterm in H; try simpl.
    { inversion H.
    }
    { assumption.
    }
  - apply WHILE_true_nonterm in H; try simpl.
    ...
    { try assumption. unfold bequiv. simpl. reflexivity.
    }
Qed.
(** [] *)
...
Proof.
   intros. split; intro H.
     ...
     - (* <- *)
       replace st' with (t_update st' X (aeval st' X)).
...
Proof.
  ...
  split; intros.
  - replace st' with (t_update st' X (aeval st' X)).
    + inversion H0. subst. apply E_Ass. unfold aequiv in H. auto.
    + apply functional_extensionality. intro. rewrite t_update_same. reflexivity.
  - inversion H0; subst. rewrite <- H. simpl. replace (t_update st X (st X)) with st.
    + constructor.
    + apply functional_extensionality. intro. rewrite t_update_same; reflexivity.
Qed.
(** [] *)
...
Definition equiv_classes : list (list com) :=
  [[prog_a;prog_d];[prog_b;prog_e;prog_f];[prog_c;prog_g;prog_h];[prog_i]].
(** [] *)
...
Proof.
  unfold bequiv,cequiv.
  intros b b' c1 c1' c2 c2' Hb1e Hc1e Hc1e' st st'. split; intros Hce.
  - remember (IFB b THEN c1 ELSE c2 FI) as cif eqn:Heqcif.
    induction Hce; inversion Heqcif; subst.
    + apply E_IfTrue. rewrite <- Hb1e. apply H. apply Hc1e. apply Hce.
    + apply E_IfFalse. rewrite <- Hb1e. apply H. apply Hc1e'. apply Hce.
  - remember (IFB b' THEN c1' ELSE c2' FI) as cif eqn:Heqcif.
    induction Hce; inversion Heqcif; subst.
    + apply E_IfTrue. 
        { rewrite -> Hb1e. apply H.
        }
        { apply Hc1e. apply Hce.
        }
    + apply E_IfFalse.
        { rewrite -> Hb1e. apply H.
        }
...
Proof.
  ...
  - (* WHILE *)
      assert (bequiv b (fold_constants_bexp b)).
      + apply fold_constants_bexp_sound.
      + destruct (fold_constants_bexp b) eqn:Heq; try (apply CWhile_congruence; assumption).
...
Proof.
  ...
  assert (WHILE BTrue DO SKIP END / { --> 0 } \\ { --> 0 }).
  - apply H. apply E_Skip.
  - remember (WHILE BTrue DO SKIP END) as b.
    induction H0; inversion Heqb.
    + rewrite H2 in H0. inversion H0.
    + rewrite H2 in H0. subst. apply IHceval2. apply Heqb.
Qed.
(** [] *)
...
Proof.
intros. simpl. apply E_Havoc. Qed.
Example havoc_example2 :
  (SKIP;; HAVOC Z) / { --> 0 } \\ { Z --> 42 }.
Proof.
intros. simpl. apply E_Seq with { --> 0 }.
  - constructor.
  - apply E_Havoc.
Qed.
(** [] *)
...
(** $Date: 2017-10-17 21:18:22 -0400 (Tue, 17 Oct 2017) $ *)
