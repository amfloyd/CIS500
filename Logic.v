...
Proof.
  ...
  - destruct m as [| m'].
    + reflexivity.
    + Search plus. rewrite -> plus_comm in H. inversion H.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q HP. unfold not. intros HQ HNA. apply HQ.
  apply HP. apply HNA.
  Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P [HP HNA]. unfold not in HNA. apply HNA. apply HP. Qed.
(** [] *)
...
Proof.
  intros X P H. unfold not. intros [x E]. apply E. apply H. Qed.
(** [] *)
...
Proof.
   intros X P Q. split.
  - intros [x [E | E']]. left. exists x. apply E.
  ...
  - induction l as [|x' l' IHl'].
    ...
    + intros [H1 | H1].
      ...
      { destruct IHl'.
        ...
        { simpl. right. destruct H as [H1' H2']. apply H2'.
        }
        }
      }
  - intros [H1 H2]. induction l as [| x' l' IHl'].
    + simpl. destruct H2 as [H1' H2']. simpl in H2'. apply H2'. 
    + simpl. destruct H2 as [H2' H2'']. inversion H2''.
      { left. rewrite -> H. apply H2'.
      }
...
Proof.
  ...
  - induction l as [|x' l'' IHl''].
    ...
    + simpl. intros [H1 | H2].
      ...
      { Search or. apply or_assoc. right. apply IHl''. apply H2.
      }
  - induction l as [|x' l'' IHl''].
    + simpl. intros [H1 | H2].
      { destruct H1.
...
Proof.
  ...
  - induction l as [|x' l' IHl'].
    ...
    + simpl. intros [H2 H3]. intros x. intros [H4 | H5].
      { rewrite <- H4. apply H2.
      }
      { apply IHl'. 
        { apply H3.
...
Proof. intros Podd Peven. intros n. intros H. intros H1. unfold combine_odd_even. destruct oddb.
  - apply H. reflexivity.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros Podd Peven n. intros H. intros H1. unfold combine_odd_even in H. destruct oddb.
  - apply H.
  - inversion H1.
Qed.
...
Lemma helper: forall (X: Type)(l1 l2: list X), rev_append l1 l2= rev l1 ++ l2.
Proof. intros X l1. induction l1 as [| x1' l1'].
  - simpl. reflexivity.
  - intros l2. induction l2 as [| x2' l2'].
    + simpl. rewrite (IHl1'(x1'::nil)). Search app. rewrite -> app_nil_r. reflexivity.
    + simpl. rewrite (IHl1'(x1'::x2'::l2')). Search app. rewrite <- app_assoc. simpl. reflexivity.
Qed.
Lemma tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof. intros X. apply functional_extensionality. unfold tr_rev.
  induction x as [| x' IHx'].
  - simpl. reflexivity.
  - simpl. rewrite <- helper. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite evenb_S. destruct evenb.
    ...
    + simpl. destruct IHn'. exists (S x). simpl. rewrite <- H. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct b1.
    + intros H3. destruct  b2.
      { reflexivity.
      }
      { inversion H3. inversion H0.
      }
    + intros H4. destruct b2.
...
Proof.
  ...
  - destruct b1.
    ...
    + intros H2. right. destruct H2. simpl. reflexivity.
  - intros [H | H].
      { rewrite H. simpl. reflexivity.
      }
      { rewrite H. Search bool. assert (H1: b1 || true = true || b1).
        { simpl. Search or. destruct b1.
          { reflexivity. }
          { simpl. reflexivity. }
        }
...
Proof.
  intros x y. split. 
  - intros H1 H2. rewrite H2 in H1. Search beq_nat. rewrite <- beq_nat_refl in H1. inversion H1.
  - destruct beq_nat.
    { intros H. unfold not in H. exfalso. destruct H. induction x.
      { induction y.
        { reflexivity.
        }
        { rewrite <- IHy. Admitted.
(** [] *)
...
Proof.
 intros A beq H1. induction l1 as [| h l1' IHl1'].
