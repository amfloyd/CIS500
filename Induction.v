...
(** For the [Require Export] to work, you first need to use
    ...
    file.  There are two ways to do it:
     ...
     - From the command line: Either
       ...
       (assuming you've downloaded the whole LF directory and 
       have a working 'make' command) or
...
(* In destruct we get stuck because of the lack of a hypothesis which is solved in Induction because it automatically
 assigns the hypothesis which helps us prove the theorem.*)
*)
(** [] *)
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    Proof: (* 
      ...
      - First, suppose [n=0]
          0 + m = m + 0.
        By the definition of [+] and [plus_n_O] we get-
          m=m
      - Next, suppose [n=Sn']
        Sn' + m = m + Sn'
        By the definition of [+],
          S (n' + m) = m + Sn'
          From the hypothesis, n' + m = m + n' so,
            S (m + n') = m + Sn'
            From the definition of plus_n_Sm we get,
              m + Sn' = m + Sn'
      Hence Proved
 *)
...
(** Write an informal proof of the following theorem, using the
    ...
    Proof: (* 
      ...
      - First, suppose [n=0]
          true = beq_nat 0 0
          By the definition of beg_nat we get,
            true = true
      - Second, suppose [n = Sn'] and assume that it holds true for [n=n']
          true = beq_nat Sn' Sn'
          By the definition of beq_nat we get
            true = beq_nat n' n'
            By the definition of the hypothesis we get,
              beq_nat n' n' = beq_nat n' n'
    Hence Proved
 *)
[] *)

(** Theorem check: forall n:nat,
     true = beq_nat n n.
...
Proof.
  intros n m p.
  rewrite <- plus_comm. assert (H: p + n = n + p). { rewrite -> plus_comm. reflexivity. }
...
Proof.
  ...
  - simpl. rewrite -> IHm'. rewrite <- helper. reflexivity.
  Qed.
(** [] *)
